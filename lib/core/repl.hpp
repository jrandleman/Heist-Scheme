...
#ifndef HEIST_REPL_HPP_
#define HEIST_REPL_HPP_
        ...
        try {
          ...
          if(eval_throw == SCM_EXCEPT::EXIT) { 
            if(!GLOBALS::HEIST_EXIT_CODE) {
              ...
              PRINT_ERR("HEIST SCHEME REPL TERMINATION: EXIT FAILURE (" << GLOBALS::HEIST_EXIT_CODE << ")!");
              puts("");
            }
            return GLOBALS::HEIST_EXIT_CODE; 
          }
          if(eval_throw == SCM_EXCEPT::JUMP)
            PRINT_ERR("Uncaught JUMP procedure! JUMPed value: " 
              << PROFILE(GLOBALS::JUMP_GLOBAL_PRIMITIVE_ARGUMENT));
          print_repl_newline();
        } catch(...) {

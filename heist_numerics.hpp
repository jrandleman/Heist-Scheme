...
public:
  ...
  Scm_numeric()                     noexcept = default;
  Scm_numeric(const Scm_numeric& s) noexcept = default;
  Scm_numeric(Scm_numeric&& s)      noexcept = default;
  ~Scm_numeric()                    noexcept = default;
  ...
  friend std::ostream& operator<<(std::ostream& outs, const Scm_numeric& s) noexcept;
  ...
  // assignment operator
  Scm_numeric& operator=(const Scm_numeric& s) noexcept = default;
  Scm_numeric& operator=(Scm_numeric&& s)      noexcept = default;
...
// ostream put operator
std::ostream& operator<<(std::ostream& outs, const Scm_numeric& s) noexcept {
  outs << s.cio_str();
  return outs;
}
...
Scm_numeric::exact_t Scm_numeric::big_int_abs_val_mul(const exact_t& a, const exact_t& b) const noexcept {
  ...
  std::vector<exact_t> adders(small.size()); 
  char carry_over = 0;
  size_type i = 0;
  ...
  for(auto ch1 = small.rbegin(); ch1 != end1; ++ch1, ++i, carry_over=0) { 
    ...
    std::reverse(adders[i].begin(),adders[i].end()); // mul'd from right to left
  }
  // add the results of mul'ing each small# digit by the big#, & return their sum
  exact_t product("0");
  for(const auto& adder : adders)
    product = big_int_abs_val_sum(product,adder);
  return product;
}
...
Scm_numeric::exact_t Scm_numeric::big_int_abs_val_diff(const exact_t& a, const exact_t& b) const noexcept {
  ...
  for(auto ch2 = small.rbegin(); ch1 != end1; ++ch1, ++ch2) {
    ...
    carry_over = (big_digit < small_digit) ? 1 : 0;
...
void Scm_numeric::trim_data_string(exact_t& num_str) noexcept {
  ...
  // erase prefixing 0's
  while(trimmed[i+1] && trimmed[i] == '0' && trimmed[i+1] == '0') 
    trimmed.erase(i,1);
...
Scm_numeric::precisions Scm_numeric::confirm_valid_string(const exact_t& num_str) noexcept {
  ...
  while(str[i]) {
    if(str[i] == '/') {
      if(found_division)    return precisions::invalid; // dual division
      if(found_decimal)     return precisions::invalid; // can't have inexact & exact
      if(found_exponential) return precisions::invalid; // can't have inexact & exact
      found_division = true;
    } else if(str[i] == '.') {
      if(found_decimal)     return precisions::invalid; // dual decimal points
      if(found_division)    return precisions::invalid; // can't have inexact & exact
      if(found_exponential) return precisions::invalid; // can't have a decimal after the exponential
      found_decimal = true;
    } else if(str[i]=='e' || str[i]=='E') {
      if(found_exponential) return precisions::invalid; // dual exponents
      if(found_division)    return precisions::invalid; // can't have inexact & exact

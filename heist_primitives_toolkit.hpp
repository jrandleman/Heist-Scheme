...
const auto VOID_DATA_EXPRESSION = scm_list({VOID_DATA_OBJECT});
const auto EMPTY_LIST_EXPRESSION = scm_list({symconst::emptylist});
...
bool primitive_data_is_a_procedure(const data& d)noexcept{
  return d.is_type(types::exp) && 
    ...
    (d.value.exp[0].value.sym == PROCEDURE_TAG || 
     d.value.exp[0].value.sym == PRIMITIVE_TAG);
}
...
bool data_is_empty(const data& d)noexcept{
  return data_is_the_empty_expression(d) || 
    (d.is_type(types::str) && d.value.str->empty()) || 
    (d.is_type(types::vec) && d.value.vec->empty());
}
...
bool prm_compare_EXPRs(const scm_list& l1, const scm_list& l2) {
  ...
  for(size_type i = 0, n = l1.size(); i < n; ++i) {
    ...
    if(l1[i].is_type(types::exp)) {            // compare sub-lists
      if(!prm_compare_EXPRs(l1[i].value.exp,l2[i].value.exp))
        return false;
...
bool prm_compare_PAIRs(const par_type& p1, const par_type& p2) {
  ...
  if(p1_car.is_type(types::exp)) {
    if(!prm_compare_EXPRs(p1_car.value.exp, p2_car.value.exp)) return false;
  ...
  if(p1_cdr.is_type(types::exp)) {
    if(!prm_compare_EXPRs(p1_cdr.value.exp, p2_cdr.value.exp)) return false;
...
data mk_string_from_generated_chrs(const scm_list& sequence, const scm_list& args,
  ...
  scm_string str_val;
  for(const auto& ch : sequence) str_val += ch.value.chr;
...
size_type primitive_get_if_valid_str_or_vec_idx(const scm_list& args,     const char* name, 
  ...
  const size_type i = (size_type)args[idx_pos].value.num.extract_inexact();
  const size_type l = (args[sequence_pos].value.*seq_ptr)->size() ;
  if(i >= l)
...
void primitive_FOLD_sequence_accumulator(scm_list& sequences,     scm_list& proc, 
  ...
  if((sequences[0].value.*seq_ptr)->empty()) return;
  if(folding_left) {
    // for each ith element
    for(size_type i = 0, n = (sequences[0].value.*seq_ptr)->size(); i < n; ++i){
      // in each sequence
      scm_list args;
      for(auto& sequence : sequences)
        args.push_back((sequence.value.*seq_ptr)->operator[](i)); // extract the ith elements
    ...
    for(size_type i = (sequences[0].value.*seq_ptr)->size(); i-- > 0;){
      // in each sequence
      scm_list args;
      for(auto& sequence : sequences)
        args.push_back((sequence.value.*seq_ptr)->operator[](i)); // extract the ith elements
  ...
  for(size_type i = 0, n = sequences.size(); i < n; ++i) {
    if(!sequences[i].is_type(seq_type))
      THROW_ERR('\''<<name<<" arg #" << i+3 << ' ' // sequences begin at idx 2
        ...
        << format << FCN_ERR(name,args));
    if(i == 0) length0 = (sequences[0].value.*seq_ptr)->size();
    else if(length0 != (sequences[i].value.*seq_ptr)->size())
...
data primitive_STATIC_SEQUENCE_FOLD_template(scm_list& args,       const char* name, 
  ...
  primitive_FOLD_sequence_accumulator(sequences,args[0].value.exp,init_val,
...
void primitive_MAP_sequence_constructor(scm_list& sequences,       scm_list& proc, 
  ...
  const size_type total_elts = (sequences[0].value.*seq_ptr)->size();
  // Traverse each elt
  for(size_type i = 0; i < total_elts; ++i) {
    ...
    for(size_type j = 0; j < total_sequences; ++j)
      args[j] = (sequences[j].value.*seq_ptr)->operator[](i);
    // Push the mapped result of each elt
    mapped_sequence.push_back(data_cast(execute_application(std::move(proc),args,env)));
  }
}
...
data primitive_STATIC_SEQUENCE_MAP_template(scm_list& args,      const char* name, 
  ...
  primitive_MAP_sequence_constructor(sequences,args[0].value.exp,
...
data primitive_STATIC_SEQUENCE_FOR_EACH_template(scm_list& args,       const char* name, 
  ...
  primitive_confirm_same_sized_sequences(sequences,name,format,args,seq_type,
                                                        seq_name,seq_ptr);
  // Apply the procedure on each elt of each sequence & store the result
  primitive_FOR_EACH_sequence_applicator(sequences, args[0].value.exp, env, seq_ptr);
...
data primitive_STATIC_SEQUENCE_COPY_BANG_template(scm_list& args,       const char* name, 
  ...
  const auto source_sequence(*(args[2].value.*seq_ptr)); // in case copying a seq to itself
  const size_type target_size = (args[0].value.*seq_ptr)->size();
  const size_type source_size = source_sequence.size();
  if(target_size-idx-1 < source_size){
    (args[0].value.*seq_ptr)->erase((args[0].value.*seq_ptr)->begin()+idx, 
                                     (args[0].value.*seq_ptr)->end());
    (args[0].value.*seq_ptr)->insert((args[0].value.*seq_ptr)->end(), 
                                     source_sequence.begin(), source_sequence.end());
  } else {
    (args[0].value.*seq_ptr)->erase((args[0].value.*seq_ptr)->begin()+idx, 
                                     (args[0].value.*seq_ptr)->begin()+idx+source_size);
    (args[0].value.*seq_ptr)->insert((args[0].value.*seq_ptr)->begin()+idx, 
                                     source_sequence.begin(), source_sequence.end());
  }
  return VOID_DATA_OBJECT;
}
...
size_type primitive_get_if_valid_string_idx(const scm_list& args,const char* name, 
                                                                 const char* layout){
  return primitive_get_if_valid_str_or_vec_idx(args,name,layout,"string",1,0,
                                                     &data_value_field::str);
}
...
data prm_trim_right_of_string(scm_list& args, env_type& env) {
  if(args[0].value.str->empty()) return make_str("");
  const size_type n = args[0].value.str->size();
  scm_string str(*args[0].value.str);
  size_type i = n-1;
  if(args.size() == 1) { // no predicate given, trim whitespace
...
data prm_string_contains_template(scm_list& args,     const char* name, 
  ...
  size_type pos;
  if(from_left)
    pos = args[0].value.str->find(*args[1].value.str);
  else
    pos = args[0].value.str->rfind(*args[1].value.str);
  ...
  if(args.size() == 3) { // confirm proper grammar
    if(!args[2].is_type(types::sym))
      THROW_ERR("'string-join 3rd arg "<<PROFILE(args[2])<<" isn't a symbol:" 
        << format << FCN_ERR("string-join", args));
    if(args[2].value.sym == "suffix") grammar = STRING_GRAMMARS::SUFFIX;
    else if(args[2].value.sym == "prefix") grammar = STRING_GRAMMARS::PREFIX;
    else if(args[2].value.sym != "infix") {
...
data primitive_LIST_STAR_to_CONS_constructor(const scm_node& obj, const scm_node& null_obj){
  ...
  data new_pair = data(make_par());
  new_pair.value.par->first = *obj;
  new_pair.value.par->second = primitive_LIST_STAR_to_CONS_constructor(obj+1,null_obj);
  return new_pair;
}
...
data primitive_CIRCULAR_LIST_to_CONS_constructor(const scm_node& obj,       const scm_node& null_obj, 
  ...
  new_pair.value.par->first = *obj;
  if(obj+1 == null_obj) {
    if(past_head) new_pair.value.par->second = head;
    else          new_pair.value.par->second = new_pair;
    return new_pair;
  }
  if(!past_head) {
    new_pair.value.par->second = primitive_CIRCULAR_LIST_to_CONS_constructor(obj+1,null_obj,new_pair,true);
  } else {
    new_pair.value.par->second = primitive_CIRCULAR_LIST_to_CONS_constructor(obj+1,null_obj,head,true);
  }
  return new_pair;
}
...
void primitive_list_LENGTH_computation(const data& curr_pair, num_type& exact_count, 
  ...
  if(curr_pair.is_type(types::par)) 
    primitive_list_LENGTH_computation(curr_pair.value.par->second, exact_count, count+1);
...
void primitive_MAP_BANG_list_constructor(scm_list& curr_pairs, scm_list& proc, 
  ...
  if(check_empty_list_else_acquire_cars_advance_cdrs(curr_pairs,args)) return;
  // Execute proc, store result, & recurse down the rest of the lists
  map_to.value.par->first = data_cast(execute_application(std::move(proc),args,env));
...
size_type primitive_get_if_valid_vector_idx(const scm_list& args,const char* name, 
                                                                 const char* layout) {
  return primitive_get_if_valid_str_or_vec_idx(args,name,layout,"vector",1,0,
                                                      &data_value_field::str);
}
...
data primtive_compute_seq_length(scm_list& args, const char* name, const char* format){
  switch(is_proper_sequence(args[0],args,name,format)) {
    case heist_sequence::vec: return num_type(args[0].value.vec->size());
    case heist_sequence::str: return num_type(args[0].value.str->size());
    case heist_sequence::nul: return num_type();
    default:
      num_type count;
      primitive_list_LENGTH_computation(args[0].value.par->second,count);
      return count;
  }
}
...
data primitive_list_filter_logic(scm_list& args, env_type& env){
  ...
  primitive_FILTER_list_constructor(args[1],args[0].value.exp,filtered_list,env);
...
data primitive_list_copy_logic(const data& curr_pair){
  ...
  new_pair.value.par->second = primitive_list_copy_logic(curr_pair.value.par->second);
  return new_pair;
}
...
data primitive_list_reverse_copy_logic(data& d){
  ...
  shallow_unpack_list_into_exp(d, par_as_exp);
  std::reverse(par_as_exp.begin(),par_as_exp.end());
  return primitive_LIST_to_CONS_constructor(par_as_exp.begin(), 
                                            par_as_exp.end());
}
...
data primitive_list_count_logic(scm_list& args, env_type& env){
  num_type count;
  primitive_LIST_COUNT_computation(args[1],count,args[0].value.exp,env);
  return count;
}
...
data primitive_list_ref_seeker(const data& curr_pair, const size_type& idx, const char* format,
  ...
  if(pos == idx) return curr_pair.value.par->first;
  return primitive_list_ref_seeker(curr_pair.value.par->second,idx,format,args,pos+1);
}
...
data primitive_substring_logic(scm_list& args, const char* format) {
  ...
  // confirm index falls w/in range of the sequence
  const auto end = (size_type)args[2].value.num.extract_inexact();
  if(end > args[0].value.str->size())
    THROW_ERR("'slice <string> received out of range <end-index> " << end << "\n     for string "
      <<args[0]<<" of size "<< args[0].value.str->size()<<'!' << format 
  ...
  return make_str(args[0].value.str->substr(start,end-start));
}
...
void primitive_list_set_index_applicator(data& curr_pair, const size_type& idx, const char* format, 
  ...
  if(pos == idx) {
    curr_pair.value.par->first = args[2];
    return;
  }
  primitive_list_set_index_applicator(curr_pair.value.par->second,idx,format,args,pos+1);
}
...
data make_deep_copy_of_list(data& curr_pair) {
  ...
  data new_pair = data(make_par());
  new_pair.value.par->first = curr_pair.value.par->first;
  new_pair.value.par->second = make_deep_copy_of_list(curr_pair.value.par->second);
  return new_pair;
}
...
void primitive_appended_deep_copy_of_RHS_to_mutated_LHS(data& LHS, data& RHS) {
  ...
  primitive_appended_deep_copy_of_RHS_to_mutated_LHS(LHS.value.par->second,RHS);
}
...
data primitive_vector_append_logic(scm_list& args, const char* format){
  ...
  for(size_type i = 0, n = args.size()-1; i < n; ++i) {
    ...
    appended_vector.insert(appended_vector.end(), args[i].value.vec->begin(),
                                                  args[i].value.vec->end());
  }
  if(appended_vector.empty()) return *args.rbegin();
  if(!args.rbegin()->is_type(types::vec))
    appended_vector.push_back(*args.rbegin());
  else
    appended_vector.insert(appended_vector.end(), args.rbegin()->value.vec->begin(),
                                                  args.rbegin()->value.vec->end());
...
data primitive_string_append_logic(scm_list& args, const char* format){
  ...
  if(i != MAX_SIZE_TYPE) {
    ...
    return make_str(str_val + char(args.rbegin()->value.chr));
  }
  return make_str(str_val + *args.rbegin()->value.str);
}
...
data primitive_remove_list_logic(data& curr_pair, scm_list& proc, env_type& env) {
  if(curr_pair.is_type(types::par)) {
    ...
    if(is_false_scm_condition(proc,pruning_args,env)){
      data new_pair = data(make_par());
      new_pair.value.par->first = curr_pair.value.par->first;
      new_pair.value.par->second = primitive_remove_list_logic(curr_pair.value.par->second,proc,env);
      return new_pair;
    }
    return primitive_remove_list_logic(curr_pair.value.par->second, proc, env); 
  }
...
// ************************ "delete" helper ************************
template<size_type(*get_idx_if_valid)(const scm_list&,const char*,const char*),
         typename SEQUENCE_PTR, typename SEQUENCE_CTOR>
data primitive_delete_STATIC_SEQUENCE_logic(scm_list& args, const char* format,
                                            SEQUENCE_PTR seq_ptr, SEQUENCE_CTOR make_sequence){
  auto new_sequence(*(args[0].value.*seq_ptr));
  new_sequence.erase(new_sequence.begin()+get_idx_if_valid(args,"delete",format));
  return make_sequence(new_sequence);
}
...
data primitive_list_delete_logic(scm_list& args, const char* format){
  ...
  const auto idx = (size_type)args[1].value.num.extract_inexact();
...
data primitive_list_init_logic(const data& curr_pair){
  ...
  data new_pair = data(make_par());
  new_pair.value.par->first = curr_pair.value.par->first;
  new_pair.value.par->second = primitive_list_init_logic(curr_pair.value.par->second);
  return new_pair;
}
...
data primitive_STATIC_SEQUENCE_sequence_eq_logic(scm_list& args, const char* format,
  ...
  for(size_type i = 1, n = total_sequences; i < n; ++i) {
    ...
    same_sizes = ((args[i].value.*seq_ptr)->size() == 
                  (args[1].value.*seq_ptr)->size() && same_sizes);
  }
  ...
  const size_type total_elements = (args[1].value.*seq_ptr)->size();
  auto& proc = args[0].value.exp;
  ...
  for(size_type i = 0; i < total_elements; ++i) { // for each element
    for(size_type j = 1; j < total_sequences; ++j)     // in each sequence
      sequence_args[j-1] = (args[j].value.*seq_ptr)->operator[](i);
...
template<data(*primitive_vector_logic)(scm_list&,decltype(make_vec),scm_list&,const char*,const char*),
         ...
         data(*primitive_list_logic)(scm_list&,decltype(primitive_LIST_to_CONS_constructor),scm_list&,const char*,const char*)>
data primitive_take_drop_template(scm_list& args,const char* name,const char* format){
  if(args.size() != 2) 
    THROW_ERR('\''<<name<<" received incorrect # of args (given " 
      ...
      << FCN_ERR(name,args));
  switch(is_proper_sequence(args[0],args,name,format)){
    case heist_sequence::vec: return primitive_vector_logic(*args[0].value.vec,make_vec,args,format,"vector");
    case heist_sequence::str: return primitive_string_logic(*args[0].value.str,make_str,args,format,"string");
    default:
...
template<data(*primitive_vector_logic)(scm_list&,decltype(make_vec),scm_list&,env_type&),
         ...
         data(*primitive_list_logic)(scm_list&,decltype(primitive_LIST_to_CONS_constructor),scm_list&,env_type&)>
data primitive_take_drop_while_template(scm_list& args,const char* name,const char* format){
  ...
  switch(is_proper_sequence(args[1],args,name,format)){
    case heist_sequence::vec: return primitive_vector_logic(*args[1].value.vec,make_vec,args[0].value.exp,env);
    case heist_sequence::str: return primitive_string_logic(*args[1].value.str,make_str,args[0].value.exp,env);
    default:
      scm_list flattened_list;
      shallow_unpack_list_into_exp(args[1], flattened_list);
      return primitive_list_logic(flattened_list,primitive_LIST_to_CONS_constructor,args[0].value.exp,env);
  }
}
...
size_type confirm_proper_STATIC_SEQUENCE_any_every_args(scm_list& args,
  ...
  for(size_type i = 1, n = args.size(); i < n; ++i) {
    ...
    if constexpr (SEQUENCE_TYPE == types::vec || SEQUENCE_TYPE == types::str) { 
      ...
      if((args[i].value.*seq_ptr)->size() < min_sequence_length) 
        min_sequence_length = (args[i].value.*seq_ptr)->size();
    } else {
      if(args[i].value.exp.size() < min_sequence_length) 
        min_sequence_length = args[i].value.exp.size();
    }
  }
...
bool convert_lists_to_exp_matrix_and_return_if_empty(scm_list& args,   scm_list& list_exps, 
  ...
  shallow_unpack_list_into_exp(args[1], list_exps[1].value.exp);
  for(size_type i = 2, n = args.size(); i < n; ++i) {
    ...
    shallow_unpack_list_into_exp(args[i], list_exps[i].value.exp);
  }
  return false;
}
...
data primitive_STATIC_SEQUENCE_any_logic(scm_list& args,       env_type& env, 
  ...
  const size_type total_sequences = args.size();
  scm_list& proc = args[0].value.exp;
  ...
  for(size_type i = 0; i < min_length; ++i){
    ...
    for(size_type j = 1; j < total_sequences; ++j) {
      if constexpr (SEQUENCE_TYPE == types::vec || SEQUENCE_TYPE == types::str) {
        any_args[j-1] = (args[j].value.*seq_ptr)->operator[](i);
      } else { // lists aren't passed via a smart pointer member
        any_args[j-1] = args[j].value.exp[i];
      }
    }
    // If set of elements is true
    auto result = execute_application(std::move(proc),any_args,env);
...
void cast_scheme_sequence_to_ast(data& scm_sequence,scm_list& sequence){
  if(scm_sequence.type == types::vec) {
    ...
    for(const auto& ch : *scm_sequence.value.str)
...
data mutatable_assign_scm_sequence(data& sequence_target, data&& sequence_source){
  if(sequence_target.is_type(types::vec))
    ...
    *sequence_target.value.par = *sequence_source.value.par;
  else
    *sequence_target.value.str = *sequence_source.value.str;
  return VOID_DATA_OBJECT;
}
...
void confirm_valid_close_port_arg(const scm_list& args, const char* io_type, 
                                                        const types& t) {
  if(no_args_given(args) || args.size() != 1)
    THROW_ERR("'close-"<<io_type<<"-port received incorrect # of args:" 

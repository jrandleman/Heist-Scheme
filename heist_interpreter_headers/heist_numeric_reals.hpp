...
namespace scm_numeric {
  ...
  ******************************************************************************/
  ...
  public:
    ...
    Snum_real to_inexact()          const noexcept;
    Snum_real to_exact()            const noexcept;
    Snum_real extract_numerator()   const noexcept;
    Snum_real extract_denominator() const noexcept;
    ...
    Snum_real() noexcept {}
    Snum_real(const Snum_real& s) noexcept {*this = s;}
    Snum_real(const Snum& s) noexcept;
    Snum_real(Snum_real&& s) noexcept {*this = std::move(s);}
    ~Snum_real() noexcept;
    Snum_real(const std::string& data) noexcept {construct_number(data);}
    Snum_real(std::string data, const int& base) noexcept {*this=convert_base_N_to_dec(base,data);}
    template<typename NumericData,typename=typename std::enable_if<std::is_arithmetic<NumericData>::value,NumericData>::type>
    Snum_real(NumericData data) noexcept {construct_number(data);}
    Snum_real(const inexact_t& data) noexcept;
    ...
    friend std::ostream& operator<<(std::ostream& outs, const scm_numeric::Snum_real& n) noexcept {
      outs << n.str();
      return outs;
    }
    ...
    // assignment operator
    Snum_real& operator=(const Snum_real& s) noexcept;
    Snum_real& operator=(const Snum& s) noexcept;
    Snum_real& operator=(Snum_real&& s) noexcept;
    ...
    Snum_real  operator-()const noexcept;
    Snum_real  operator- (const Snum_real& s) const noexcept{return *this + -s;}
    Snum_real& operator-=(const Snum_real& s)noexcept{*this = *this - s; return *this;}
    Snum_real  operator* (const Snum_real& s) const noexcept;
    Snum_real& operator*=(const Snum_real& s)noexcept{*this = *this * s; return *this;}
    Snum_real  operator/ (const Snum_real& s) const noexcept;
    ...
    friend Snum_real operator*(const NumericData& lhs,const Snum_real& rhs)noexcept{return rhs * lhs;}
    template<typename NumericData,typename=typename std::enable_if<std::is_arithmetic<NumericData>::value,NumericData>::type>
    friend Snum_real operator/(const NumericData& lhs,const Snum_real& rhs)noexcept{Snum_real tmp=lhs; return tmp / rhs;}
    template<typename NumericData,typename=typename std::enable_if<std::is_arithmetic<NumericData>::value,NumericData>::type>
    friend Snum_real operator%(const NumericData& lhs,const Snum_real& rhs)noexcept{Snum_real tmp=lhs; return tmp % rhs;}
    ...
    static Snum_real random(Snum_real seed = std::chrono::system_clock::now().time_since_epoch().count())noexcept;


    // **************** MISCELLANEOUS DERIVED NUMERIC OPERATIONS ****************

    // Exponentiation
    Snum_real  expt(const Snum_real& s) const noexcept;
    Snum_real& expt_eq(const Snum_real& s)noexcept{*this = expt(s); return *this;}

    // greatest common denominator
    Snum_real gcd(const Snum_real& s) const noexcept;
    // least common multiple
    Snum_real lcm(const Snum_real& s) const noexcept;

    // exponential function
    Snum_real exp()  const noexcept {return UNDERLYING_CPP_GENERIC_FCN(12);}

    // sqrt function
    Snum_real sqrt() const noexcept {return expt(0.5L);}

    // NATURAL logarithm
    Snum_real log()  const noexcept;

    // absolute value
    Snum_real abs()  const noexcept {return (is_neg() || is_neg_inf()) ? -*this : *this;}

    // quotient & modulo of being div'd by arg
    Snum_real quotient(const Snum_real& s) const noexcept;
    Snum_real modulo  (const Snum_real& s) const noexcept;


    // **************************** ROUNDING METHODS ****************************

    Snum_real ceil()  const noexcept {return ROUNDING_GENERIC_FCN(0);}
    Snum_real floor() const noexcept {return ROUNDING_GENERIC_FCN(1);}
    Snum_real trunc() const noexcept {return ROUNDING_GENERIC_FCN(2);}
    Snum_real round() const noexcept {return ROUNDING_GENERIC_FCN(3);}


    // ****************** TRIGONOMETRIC METHODS -:- IN RADIANS ******************

    Snum_real sin()   const noexcept {return UNDERLYING_CPP_GENERIC_FCN(0);}
    Snum_real cos()   const noexcept {return UNDERLYING_CPP_GENERIC_FCN(1);}
    Snum_real tan()   const noexcept {return UNDERLYING_CPP_GENERIC_FCN(2);}
    Snum_real asin()  const noexcept {return UNDERLYING_CPP_GENERIC_FCN(3);}
    Snum_real acos()  const noexcept {return UNDERLYING_CPP_GENERIC_FCN(4);}
    Snum_real atan()  const noexcept {return UNDERLYING_CPP_GENERIC_FCN(5);}
    Snum_real sinh()  const noexcept {return UNDERLYING_CPP_GENERIC_FCN(6);}
    Snum_real cosh()  const noexcept {return UNDERLYING_CPP_GENERIC_FCN(7);}
    Snum_real tanh()  const noexcept {return UNDERLYING_CPP_GENERIC_FCN(8);}
    Snum_real asinh() const noexcept {return UNDERLYING_CPP_GENERIC_FCN(9);}
    Snum_real acosh() const noexcept {return UNDERLYING_CPP_GENERIC_FCN(10);}
    Snum_real atanh() const noexcept {return UNDERLYING_CPP_GENERIC_FCN(11);}
    Snum_real atan2(const Snum_real& denom)const noexcept;


    // ************************** COMPARISON OPERATORS **************************

    // overloaded equality/comparison operators
    bool operator==(const Snum_real& s) const noexcept;
    ...
    template<typename NumericData,typename=typename std::enable_if<std::is_arithmetic<NumericData>::value,NumericData>::type>
    friend bool operator==(const NumericData& lhs, const Snum_real& rhs) noexcept {return rhs == lhs;}
    template<typename NumericData,typename=typename std::enable_if<std::is_arithmetic<NumericData>::value,NumericData>::type>
    friend bool operator!=(const NumericData& lhs, const Snum_real& rhs) noexcept {return rhs != lhs;}
    template<typename NumericData,typename=typename std::enable_if<std::is_arithmetic<NumericData>::value,NumericData>::type>
    friend bool operator< (const NumericData& lhs, const Snum_real& rhs) noexcept {return rhs > lhs;}
    ...
    Snum_real operator~()const noexcept;
    Snum_real operator& (const Snum_real& rhs) const noexcept;
    Snum_real operator| (const Snum_real& rhs) const noexcept;
    Snum_real operator^ (const Snum_real& rhs) const noexcept;
    Snum_real operator<<(const Snum_real& rhs) const noexcept;
    Snum_real operator>>(const Snum_real& rhs) const noexcept;
    Snum_real asr       (const Snum_real& rhs) const noexcept;

    Snum_real& operator&= (const Snum_real& rhs) noexcept {*this = *this & rhs;  return *this;}
    Snum_real& operator|= (const Snum_real& rhs) noexcept {*this = *this | rhs;  return *this;}
    Snum_real& operator^= (const Snum_real& rhs) noexcept {*this = *this ^ rhs;  return *this;}
    Snum_real& operator<<=(const Snum_real& rhs) noexcept {*this = *this << rhs; return *this;}
    Snum_real& operator>>=(const Snum_real& rhs) noexcept {*this = *this >> rhs; return *this;}
    Snum_real& asr_eq     (const Snum_real& rhs) noexcept {*this = asr(rhs);     return *this;}
  ...
  private:
    ...
    void inexact_integer_to_exact_t(Snum_real& exact_num, const long double& d)const noexcept;
    ...
    Snum_real convert_base_N_to_dec(const int& base, std::string bnum) const noexcept;
    // Converts the Snum_real decimal fractional representation into the <base> # system
    std::string convert_dec_to_base_N_flonum(const int& base, const Snum_real& dnum) const noexcept;
    // Converts the Snum_real decimal representation into the <base> # system
    std::string convert_dec_to_base_N(const int& base, const Snum_real& dnum) const noexcept;
    // Evaluates the numerator & denominator of a base-N # & divides their results
    Snum_real form_decimal_fraction(const int& base, const size_type& i, const std::string& bnum, const bool& is_neg) const noexcept;
    // Evaluates the integral & fractional of a base-N # & combines their results
    Snum_real form_decimal_floating_pt(const int& base, const size_type& i, const std::string& bnum, const bool& is_neg) const noexcept;
    // Evaluates the numerator & denominator of a decimal # & divides their results
    std::string form_base_N_fraction(const int& base,const Snum_real& dnum) const noexcept;
    // Evaluates the integral & fractional of a decimal # & combines their results
    std::string form_base_N_floating_pt(const int& base, const Snum_real& dnum) const noexcept;
    // Check if any early cases of conversion can be leveraged
    std::string convert_dec_to_base_N_preliminary_check(const int& base, const Snum_real& dnum)const noexcept;
    ...
    // Convert possibly-signed binary string to a Snum_real
    Snum_real big_int_revert_BINARY_to_Snum_real(std::string bin_str, const bool& is_negative_binary) const noexcept;
    // Convert a Snum_real to a no-fraction binary string (degrades fractions to inexact_t)
    bool get_non_fraction_binary_string(Snum_real n, std::string& bit_str, const bool& is_signed) const noexcept;
    // Convert 2 Snum_reals to no-fraction binary strings (degrades fractions to inexact_t)
    bool get_non_fraction_binary_strings(const Snum_real& n1, std::string& bit_str1, const Snum_real& n2, std::string& bit_str2) const noexcept;
    // Template outlining structure of LSR & ASR (logical/arithmetic shift right)
    template<bool LOGIC_RIGHT_SHIFT> Snum_real GENERIC_SHIFT_RIGHT(const Snum_real& rhs) const noexcept;
    ...
    std::pair<std::pair<size_type,size_type>,Snum_real::status> decompose_int_into_SIZE_TYPE_MAX() const noexcept;
    // Perform the "repeated squares" algorithm (rapid exponentiation)
    Snum_real repeated_squares(const Snum_real& a, const size_type& b) const noexcept;

    // Generic rounding fcn template for ceil, floor, trunc, & round
    Snum_real ROUNDING_GENERIC_FCN(const size_type ROUNDING_TYPE_ID) const noexcept;
    // Generic cpp-fcn-invocation template
    Snum_real UNDERLYING_CPP_GENERIC_FCN(const size_type CPP_FCN_TYPE_ID) const noexcept;
    // Float Extraction for underlying cpp-fcn invocations
    bool extract_float_val(Snum_real& tmp, inexact_t& value) const noexcept;
  ...
  ******************************************************************************/
  ...
  Snum_real Snum_real::random(Snum_real seed) noexcept {
  ...
  ******************************************************************************/
  ...
  bool Snum_real::is_integer() const noexcept {
  ...
  Snum_real Snum_real::to_inexact() const noexcept {
  ...
  Snum_real Snum_real::to_exact() const noexcept {
  ...
  Snum_real Snum_real::extract_numerator() const noexcept {
    auto tmp = to_exact();
    if(tmp.is_zero()) return tmp;
    if(tmp.stat != status::success || tmp.is_float) {
      tmp.set_failed_status(); return tmp;
    }
    if(!tmp.ncapacity || !tmp.nlen) return Snum_real(); // 0
    tmp.resize_denominator(1);
    tmp.denominator[tmp.dlen++] = 1;
    return tmp;
  }
  ...
  Snum_real Snum_real::extract_denominator() const noexcept {
    auto tmp = to_exact().abs();
    if(tmp.is_zero()) return Snum_real("1");
    if(tmp.stat != status::success || tmp.is_float) {
      tmp.set_failed_status(); return tmp;
    }
  ...
  ******************************************************************************/
  ...
  Snum_real::inexact_t Snum_real::extract_inexact() const noexcept {
  ...
  std::string Snum_real::extract_exact() const noexcept {
    if(is_zero()) return "0";
    auto tmp = to_exact();
    if(tmp.stat != status::success) {
  ...
  ******************************************************************************/
  ...
  void Snum_real::set_zero() noexcept {
    sign = signs::zero;
    resize_numerator(1);
    numerator[nlen++] = 0;
  ...
  ******************************************************************************/
  ...
  void Snum_real::resize_numerator(const size_type& new_size)noexcept{
  ...
  void Snum_real::resize_denominator(const size_type& new_size)noexcept{
  ...
  ******************************************************************************/
  ...
  std::string Snum_real::convert_exact_to_string(const exact_t exact_num, const size_type& len)const noexcept{
    std::string str;
    str.reserve(len);
  ...
  long double Snum_real::exact_to_ld(const exact_t exact_num, const size_type& len) const noexcept {
    long double num = 0;
    for(size_type i = 0; i != len; ++i)
  ...
  template<bool COERCING_NUMERATOR>
  void Snum_real::inexact_integer_to_exact_t(Snum_real& exact_num, const long double& d)const noexcept{
  ...
  std::string Snum_real::convert_numeric_to_str(const inexact_t& n) const noexcept {
    char outs[64];
  ...
  ******************************************************************************/
  ...
  // Simplify the current number (simplifies exact number/converts to float as needed)
  void Snum_real::simplify_numerics() noexcept {
  ...
  // Adjust the current number's float invariants
  void Snum_real::adjust_float_invariants() noexcept {
  ...
  constexpr void Snum_real::set_failed_status() noexcept {
  ...
  ******************************************************************************/
  ...
  // assignment operator
  Snum_real& Snum_real::operator=(const Snum_real& s) noexcept {
  ...
  Snum_real& Snum_real::operator=(Snum_real&& s) noexcept {
    if(this == &s) return *this;
    ...
    s.sign = signs::zero; // moved from Snum_real's become 0
    return *this;
  }
  ...
  ******************************************************************************/
  ...
  Snum_real Snum_real::operator+(const Snum_real& s) const noexcept {
    Snum_real tmp;
  ...
  ******************************************************************************/
  ...
  Snum_real Snum_real::operator-() const noexcept {
    auto tmp = *this;
  ...
  ******************************************************************************/
  ...
  Snum_real Snum_real::operator*(const Snum_real& s) const noexcept {
    Snum_real tmp;
    // n * 0 = 0
    if(is_zero() || s.is_zero()) return Snum_real();
    if(stat != status::success || s.stat != status::success) {
      // NaN if either NaN
      if(is_nan() || s.is_nan()) {
        tmp.stat = status::nan; return tmp;
  ...
  ******************************************************************************/
  ...
  Snum_real Snum_real::operator/(const Snum_real& s) const noexcept {
    ...
    // real# / inf = 0
    if(tmp.is_pos_inf() || tmp.is_neg_inf()) return Snum_real();
  ...
  ******************************************************************************/
  ...
  Snum_real Snum_real::operator%(const Snum_real& s) const noexcept {
    ...
    // n % n = 0 % n = 0
    if(abs_this == abs_s || is_zero()) return Snum_real();
    // for n < m, n % m = n
    ...
    // use the bignum variant
      ...
      if(remainder.size() == 1 && remainder[0] == 0) return Snum_real(); // remainder is 0
  ...
  ******************************************************************************/
  ...
  Snum_real Snum_real::expt(const Snum_real& s) const noexcept {
    Snum_real tmp, pow(s);
    // n^NaN = NaN^n = NaN
    ...
    if(!pow.is_float) {
      ...
      if(decomp_status != status::success){
        ...
        if(decomp_status == status::ninf) return Snum_real();                // n^-inf = 0
  ...
  ******************************************************************************/
  ...
  Snum_real Snum_real::gcd(const Snum_real& s) const noexcept {
    Snum_real tmp;
  ...
  ******************************************************************************/
  ...
  Snum_real Snum_real::lcm(const Snum_real& s) const noexcept {
  ...
  ******************************************************************************/
  ...
  Snum_real Snum_real::log() const noexcept {
    Snum_real tmp(*this);
    ...
    return Snum_real(std::log(ln_base)); 
  }
  ...
  ******************************************************************************/
  ...
  Snum_real Snum_real::quotient(const Snum_real& s) const noexcept {
    // Not handling 2 ints
    if(!(is_exact() && is_integer()) || !(s.is_exact() && s.is_integer())) {
      auto tmp = *this / s;
    ...
    // Handling 2 ints
      ...
      for(size_type i = 0; i < s.nlen; ++i) bVec[i] = s.numerator[i];
      BIGNUM_UNSIGNED_DIVIDE_core(aVec,bVec,quotient,remainder);
      if(quotient.size() == 1 && quotient[0] == 0) return Snum_real(); // quotient is 0
  ...
  Snum_real Snum_real::modulo(const Snum_real& s) const noexcept {
  ...
  ******************************************************************************/
  ...
  bool Snum_real::operator==(const Snum_real& s) const noexcept {
  ...
  ******************************************************************************/
  ...
  Snum_real Snum_real::operator<<(const Snum_real& rhs) const noexcept {
    ...
    return big_int_revert_BINARY_to_Snum_real(bin_str, bin_str[0] == '1');
  }
  ...
  ******************************************************************************/
  ...
  Snum_real Snum_real::asr(const Snum_real& rhs) const noexcept {
  ...
  ******************************************************************************/
  ...
  Snum_real Snum_real::operator~() const noexcept {
    ...
    // ~0 = -1
    if(is_zero()) return Snum_real("-1");
    // ~(-1) = 0
    if(is_negative_one()) return Snum_real(); 
    // Extract binary string
    std::string bin_str;
    ...
    // Revert binary string to a decminal reresentation
    return big_int_revert_BINARY_to_Snum_real(bin_str_result, 
      bin_str_result[0] == '1');
  }
  ...
  ******************************************************************************/
  ...
  Snum_real Snum_real::operator&(const Snum_real& rhs) const noexcept {
    ...
    // num & 0 = 0
    if(is_zero() || rhs.is_zero()) return Snum_real(); 
    // num & -1 = num & num = num
  ...
  ******************************************************************************/
  ...
  bool Snum_real::is_even() const noexcept {
  ...
  ******************************************************************************/
  ...
  bool Snum_real::is_rational() const noexcept {
  ...
  ******************************************************************************/
  ...
  std::string Snum_real::str() const noexcept {
    // Return NaN/Inf as needed
    if(stat != Snum_real::status::success) {
    ...
    if(is_zero()) {
      ...
      if(float_num >= Snum_real::RATIONALITY_LIMIT || !is_integer())
        return str;
  ...
  // get current value as a string in 'base' radix form
  std::string Snum_real::str(const int& base) const noexcept {
  ...
  ******************************************************************************/
  ...
  // Fast comparison for whether *this == -1
  bool Snum_real::is_negative_one() const noexcept {
  ...
  // Performs 2's complement negation on binary string bin_str
  void Snum_real::big_int_BINARY_2sCMPL_NEGATION(std::string& bin_str) const noexcept {
    auto i = bin_str.size();
  ...
  void Snum_real::big_int_BINARY_extend(std::string& bit_str1, std::string& bit_str2, 
  ...
  void Snum_real::signed_big_int_BINARY_extend(std::string& bit_str1, std::string& bit_str2) const noexcept {
    big_int_BINARY_extend(bit_str1,bit_str2,bit_str1[0],bit_str2[0]);
  }

  // Unsigned-extend bit_str1 & bit_str2 to match lengths
  // PRECONDITION: bit_str1 & bit_str2 MUST be NON-EMPTY!
  void Snum_real::unsigned_big_int_BINARY_extend(std::string& bit_str1, 
                                            std::string& bit_str2) const noexcept {
    big_int_BINARY_extend(bit_str1,bit_str2,'0','0');
  }

  // Convert possibly-signed binary string to a Snum_real
  Snum_real Snum_real::big_int_revert_BINARY_to_Snum_real(std::string bin_str, 
                                        const bool& is_negative_binary) const noexcept {
    if(is_negative_binary) {
      big_int_BINARY_2sCMPL_NEGATION(bin_str);
      return -(Snum_real(bin_str,2));
    }
    return Snum_real(bin_str,2);
  }

  // Convert a Snum_real to a no-fraction binary string 
  //   (degrades fractions to inexact_t & returns success status)
  bool Snum_real::get_non_fraction_binary_string(Snum_real n, std::string& bit_str, 
  ...
  // Convert 2 Snum_reals to no-fraction binary strings 
  //   (degrades fractions to inexact_t & returns success status)
  bool Snum_real::get_non_fraction_binary_strings(const Snum_real& n1, std::string& bit_str1, 
                                             const Snum_real& n2, std::string& bit_str2)const noexcept{
  ...
  Snum_real Snum_real::GENERIC_SHIFT_RIGHT(const Snum_real& rhs) const noexcept {
    // num >> inf = num >> NaN = num >> <negative-or-inexect_t> = NaN
    if(stat != status::success || rhs.stat != status::success || 
      !rhs.is_integer() || rhs.is_neg()) {
      Snum_real tmp; tmp.stat = status::nan; return tmp;
    ...
    // Revert binary string to a decminal reresentation
    return big_int_revert_BINARY_to_Snum_real(bin_str, is_neg() && bin_str[0] == '1');
  }
  ...
  ******************************************************************************/
  ...
  std::pair<std::pair<Snum_real::size_type,Snum_real::size_type>,Snum_real::status> 
  Snum_real::decompose_int_into_SIZE_TYPE_MAX() const noexcept {
    const Snum_real unum_Snum_real(SIZE_TYPE_MAX);
    const auto div_res = (*this / unum_Snum_real), mod_res = (*this % unum_Snum_real);
  ...
  Snum_real Snum_real::repeated_squares(const Snum_real& a, const size_type& b) const noexcept { // a^b
  ...
  ******************************************************************************/
  ...
  // Generic rounding fcn template for ceil, floor, trunc, & round
  Snum_real Snum_real::ROUNDING_GENERIC_FCN(const size_type ROUNDING_TYPE_ID) const noexcept {
  ...
  ******************************************************************************/
  ...
  // Returns whether big-int a > big-int b
  bool Snum_real::big_int_gt(const exact_t a, const size_type& a_len, const exact_t b, const size_type& b_len)const noexcept{
  ...
  // Returns whether big-int a < big-int b
  bool Snum_real::big_int_lt(const exact_t a, const size_type& a_len, const exact_t b, const size_type& b_len)const noexcept{
    if(a_len != b_len) return a_len < b_len;
    for(size_type i = 0; i < a_len; ++i) {
      if(a[i] != b[i]) return (a[i] < b[i]);
    }
  ...
  void Snum_real::big_int_abs_val_sum(exact_t& sum, size_type& sum_len, 
  ...
  ******************************************************************************/
  ...
  bool Snum_real::extract_float_val(Snum_real& tmp, inexact_t& value) const noexcept {
    if(is_float) {
  ...
  // Generic cpp-fcn-invocation template
  Snum_real Snum_real::UNDERLYING_CPP_GENERIC_FCN(const size_type CPP_FCN_TYPE_ID) const noexcept {
    Snum_real tmp;
  ...
  ******************************************************************************/
  ...
  Snum_real Snum_real::atan2(const Snum_real& denom) const noexcept {
    Snum_real tmp;
  ...
  ******************************************************************************/
  ...
  // rm whitespace from number string & trims padding 0's
  void Snum_real::trim_data_string(std::string& num_str) noexcept {
    std::string trimmed;
  ...
  // Confirm given string is a viable candidate to be a number
  Snum_real::precisions Snum_real::confirm_valid_string(const std::string& num_str) noexcept {
  ...
  ******************************************************************************/
  ...
  Snum_real::Snum_real(const inexact_t& data) noexcept {
    if(data == 0) return;
  ...
  // Parse the _confirmed_ valid number
  void Snum_real::parse_integer(std::string& s) noexcept {
  ...
  // Assign NaN or Inf if given such as a string, and return whether did so
  bool Snum_real::is_irrational(const std::string& num_str) noexcept {
  ...
  void Snum_real::construct_number(std::string num_str) noexcept {
    // Check if NaN or Inf
  ...
  ******************************************************************************/
  ...
  const char * base_digit_conversion(const Snum_real& digit, const int& base) noexcept {
  ...
  // Evaluates the numerator & denominator of a base-N # & divides their results
  Snum_real Snum_real::form_decimal_fraction(const int& base,const size_type& i,const std::string& bnum,const bool& is_neg)const noexcept{
    if(i == 0) {
      Snum_real tmp; tmp.stat = status::nan; return tmp; // Invalid Snum_real Given: Can't begin with '/'!
    }
  ...
  // Evaluates the integral & fractional of a base-N # & combines their results
  Snum_real Snum_real::form_decimal_floating_pt(const int& base,const size_type& i,const std::string& bnum,const bool& is_neg)const noexcept{
    auto fractional = convert_base_N_decimal_to_dec(base,bnum.substr(i+1, bnum.size()-i-1));
    if(fractional == "+nan.0") {Snum_real tmp; tmp.stat = status::nan; return tmp;}
    if(i == 0) return Snum_real("0." + fractional) * (is_neg ? -1 : 1);
    return (convert_base_N_to_dec(base,bnum.substr(0,i)) + Snum_real("0." + fractional)) * (is_neg ? -1 : 1);
  }


  // Evaluates the numerator & denominator of a decimal # & divides their results
  std::string Snum_real::form_base_N_fraction(const int& base,const Snum_real& dnum) const noexcept {
  ...
  // Evaluates the integral & fractional of a decimal # & combines their results
  std::string Snum_real::form_base_N_floating_pt(const int& base, const Snum_real& dnum) const noexcept {
  ...
  // Check if any early cases of conversion can be leveraged
  std::string Snum_real::convert_dec_to_base_N_preliminary_check(const int& base, const Snum_real& dnum)const noexcept{
    // return if invalid Snum_real base given, must be in range: [2,36]!
  ...
  ******************************************************************************/
  ...
  Snum_real Snum_real::convert_base_N_to_dec(const int& base, std::string bnum) const noexcept {
    if(!confirm_base_in_range(base)) {
      // Invalid Snum_real Base Given, Must be in Range: [2,36]!
      Snum_real tmp; tmp.stat = status::nan; return tmp; 
    }
    ...
    using exactVec_t = std::vector<exact_val_t>;
    Snum_real BASE_10_HORNER_SCHEME(const exactVec_t&,const Snum_real&)noexcept;
    void convert_string_to_exactVec(const std::string&, exactVec_t&)noexcept;
    exactVec_t baseVec;
    convert_string_to_exactVec(bnum,baseVec);
    if(is_neg) return Snum_real("-1") * BASE_10_HORNER_SCHEME(baseVec,base);
    return BASE_10_HORNER_SCHEME(baseVec,base);
  }


  std::string Snum_real::convert_dec_to_base_N_flonum(const int& base, const Snum_real& dnum)const noexcept{
    if(auto early_case = convert_dec_to_base_N_preliminary_check(base,dnum); !early_case.empty())
      return early_case;

    Snum_real D = dnum.abs();
    std::string bnum;
  ...
  std::string Snum_real::convert_dec_to_base_N(const int& base, const Snum_real& dnum) const noexcept {
    if(auto early_case = convert_dec_to_base_N_preliminary_check(base,dnum); !early_case.empty())
      return early_case;
  ...
  ******************************************************************************/
  ...
  // Erases redundant RHS padding 0s, trimming fractionals
  void Snum_real::reduce_redundant_RHS_0s(std::string& fractional) const noexcept {
  ...
  // Pad 0 to the LHS of a <base> fractional as needed for improved accuracy
  void Snum_real::pad_LHS_base_N_decimal_0s_as_needed(const int& base, const std::string& dnum, std::string& bnum) const noexcept {
    const Snum_real scm_dnum("0."+dnum);
    while(Snum_real("0."+convert_base_N_decimal_to_dec(base,'0'+bnum)) > scm_dnum)     // while padding a 0 = more accuracy
      bnum.insert(0,"0");                                                                // pad a 0
    if((Snum_real("0."+convert_base_N_decimal_to_dec(base,'0'+bnum)) - scm_dnum).abs() // if padding an extra 0
         <                                                                               // yields more accuracy than
         (Snum_real("0."+convert_base_N_decimal_to_dec(base,bnum)) - scm_dnum).abs()){ // not padding an extra zero
  ...
  ******************************************************************************/
  ...
  std::string Snum_real::convert_base_N_decimal_to_dec(const int& base, std::string bnum) const noexcept {
    ...
    auto decimals = (convert_base_N_to_dec(base,bnum) / (Snum_real(base).expt(bnum.size()))).extract_inexact();
  ...
  std::string Snum_real::convert_dec_decimal_to_base_N(const int& base, std::string dnum) const noexcept {
    // Return if nothing to convert or given an invalid "base" string of digits
    if(base == 10 || Snum_real(dnum).is_zero()) return dnum;
    ...
    // perform the above algorithm
    auto base_fractional = convert_dec_to_base_N_flonum(base, (Snum_real("0."+dnum) * 
                                                       (Snum_real(base).expt(INEXACT_PRECISION))).round());
  ...
  ******************************************************************************/
  ...
  // IE whether became a float as requested, OR resulted in +- inf.
  Snum_real::status Snum_real::coerce_fraction_to_float(inexact_t& num)const noexcept{
  ...
  ******************************************************************************/
  ...
  using exactVec_t = std::vector<Snum_real::exact_val_t>;
  ...
  namespace bit_to_decArr_conversion_helpers {
    ...
    void double_decArr(exactVec_t& decArr)noexcept{
      Snum_real::exact_val_t carry_over = 0;
  ...
  namespace dec_to_bit_string_conversion_helpers {
    void rm_prepending_0s(Snum_real::exact_t& quotient, std::size_t& decArr_len)noexcept{
      std::size_t i = 0;
    ...
    Snum_real::exact_val_t decStr_div2(Snum_real::exact_t& decArr, std::size_t& decArr_len)noexcept{
      if(!decArr_len) return 0;
      Snum_real::exact_val_t remainder = decArr[decArr_len-1] & 1;
    ...
    void repeatedDiv_recur(exactVec_t& binArr, Snum_real::exact_t decArr, std::size_t decArr_len)noexcept{
  ...
  namespace binary_division_helpers {
    ...
    void remove_prepending_0s_by_advancing_ptrs(Snum_real::exact_t& binArr, std::size_t& binArr_len)noexcept{
      std::size_t i = 0;
    ...
    bool binArr_GTE(exactVec_t& lhs, exactVec_t& rhs)noexcept{
      ...
      Snum_real::exact_t lhs_data = lhs.data(), rhs_data = rhs.data();
    ...
    void binArr_SUBTRACT(exactVec_t& lhs, const exactVec_t& rhs)noexcept{
      ...
      for(std::size_t j = rhs.size()-1; i != SIZE_TYPE_MAX && j != SIZE_TYPE_MAX; --i, --j) {
        Snum_real::exact_val_t small_digit = rhs[j] + carry_over; // carry the one (or 0) as needed
      ...
      for(; i != SIZE_TYPE_MAX; --i) {
        Snum_real::exact_val_t small_digit = carry_over;
        carry_over = lhs[i] < small_digit;
  ...
  exactVec_t b10Vec_to_b2Vec(exactVec_t& decVec)noexcept{
    Snum_real::exact_t decArr = decVec.data();
  ...
  ******************************************************************************/
  ...
  Snum_real::exact_val_t convert_exactVec_to_UNSIGNED(const exactVec_t& v)noexcept{
  ...
  // BASE N -> BASE 10
  Snum_real BASE_10_HORNER_SCHEME(const exactVec_t& baseVec, const Snum_real& Base)noexcept{
    if(baseVec.empty()) return Snum_real();

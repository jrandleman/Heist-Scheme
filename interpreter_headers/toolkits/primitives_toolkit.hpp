  ...
  void validate_oo_member_setter(scm_list& args, const char* name, const char* format) {
    ...
    if(!args[0].is_type(types::obj))
      THROW_ERR('\''<<name<<" 1st object arg "<<PROFILE(args[0])<<" isn't an object!"
        <<format<<FCN_ERR(name,args));
    if(!args[1].is_type(types::sym))
      THROW_ERR('\''<<name<<" 2nd property-name arg "<<PROFILE(args[1])<<" isn't a symbol!"
        <<format<<FCN_ERR(name,args));
    if(args[1].sym == "super")
      THROW_ERR('\''<<name<<" property <super> can't be set to a new value!"
        <<format<<FCN_ERR(name,args));
    if(args[1].sym == "prototype")
      THROW_ERR('\''<<name<<" property <prototype> can't be set to a new value!"
        <<format<<FCN_ERR(name,args));
  }
